...
def handle_calculate_budget():
    ...
    missing_fields = [
        ...
        or (isinstance(data.get(f), str) and not str(data.get(f)).strip())
    ]

    if data.get("bags") is None:
        data["bags"] = 0
    ...
    if missing_fields:
        return (
            jsonify(
                {"error": f"Campos em falta ou inválidos: {', '.join(missing_fields)}"}
            ),
            400,
        )
    ...
    try:
        ...
        budget_details = budget_service.calculate_estimated_budget(
            data, request_time_obj=current_server_time
        )
        return (
            jsonify(
                {
                    "message": "Orçamento calculado com sucesso.",
                    "original_budget_pre_vat": budget_details[
...
def handle_validate_voucher():
    ...
    if not voucher_code or original_budget_pre_vat_str is None:
        return (
            jsonify(
                {
                    "error": "Campos 'voucher_code' e 'original_budget_pre_vat' são obrigatórios."
                }
            ),
            400,
        )
    ...
    try:
        valid_voucher = vouchers_service.validate_voucher_for_use(
            voucher_code, original_budget_pre_vat
        )
        (
            final_budget_pre_vat,
            discount_amount,
        ) = vouchers_service.apply_voucher_to_budget(
            original_budget_pre_vat, valid_voucher
        )
        ...
        return (
            jsonify(
                {
                    "message": f"Voucher '{valid_voucher.code}' válido e aplicado!",
                    "valid": True,
                    "voucher_code": valid_voucher.code,
                    "original_budget_pre_vat": original_budget_pre_vat,
                    "discount_amount": discount_amount,
                    "final_budget_pre_vat": final_budget_pre_vat,
                    "vat_percentage": vat_percentage,
                    "vat_amount": vat_amount_final,
                    "total_with_vat": total_with_vat_final,
        ...
        logger.error(
            f"Erro inesperado ao validar voucher '{voucher_code}': {e}", exc_info=True
        )
